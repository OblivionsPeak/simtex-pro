export default {
  ...
  shader: `
    ...
    vec3 generate() {
      ...
      // Layer 1: Dark Shadows / Bark
      float n1 = noise(uv * 1.5);
      float mask1 = step(0.6, n1);
      
      // Layer 2: Major Leaves
      float n2 = noise(uv * 3.0 + n1 * 0.5);
      float mask2 = step(0.6, n2);
      
      // Layer 3: Branch structure (stretched noise)
      float n3 = noise(vec2(uv.x * 0.5, uv.y * 4.0));
      float mask3 = step(0.7, n3);
      
      vec3 forestGreen = vec3(0.1, 0.15, 0.05);
      vec3 tan = vec3(0.5, 0.45, 0.3);
      vec3 brown = vec3(0.25, 0.15, 0.1);
      vec3 dark = vec3(0.05, 0.05, 0.02);
